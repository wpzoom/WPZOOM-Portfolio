	...
	class WPZOOM_Featured_Posts {
		...
		public function admin_enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'buttonLabel'            => __( 'Save Order', 'wpzoom-portfolio' ),
					'buttonReorderListLabel' => __( 'Reset Order', 'wpzoom-portfolio' ),
					'reorderItemsDesc'       => __( 'Click on this button if you want to reset the default order of the items', 'wpzoom-portfolio' ),
		...
		public function get_post_time( $post ) {
			if ( '0000-00-00 00:00:00' === $post->post_date ) {
				$t_time    = $h_time = __( 'Unpublished', 'wpzoom-portfolio' );
				$time_diff = 0;
			} else {
				...
				if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS ) {
					$h_time = sprintf( __( '%s ago', 'wpzoom-portfolio' ), human_time_diff( $time ) );
				} else {
		...
		public function reorder_items() {
			...
			if ( wp_verify_nonce( $sliced['nonce_reorder_items'], $this->prefix_action( 'reorder_items' ) ) ) {
				...
				$response['success']         = true;
				$response['data']['message'] = __( 'Items reordered', 'wpzoom-portfolio' );
			}
		...
		public function render_description() {
			printf( __( '<p>Your slideshow is limited to show %d posts.</p>', 'wpzoom-portfolio' ), $this->posts_limit );
			printf( __( '<p>You can change the number of the posts in <a href="%s">Theme Options</a>.</p>', 'wpzoom-portfolio' ), menu_page_url( 'wpzoom_options', false ) );
		}
